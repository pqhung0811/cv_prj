    ...
    for cur_epoch in range(end_epoch):
        ...
        if (cur_epoch + 1) % 10 == 0:
            torch.save(
                net.state_dict(),
                os.path.join(
                    model_store_path, "rnet_model_epoch_%d.pt" % (cur_epoch + 1)
                ),
            )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train RNet", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument(
        ...
        default=os.path.join(config.ANNO_STORE_DIR, config.RNET_TRAIN_IMGLIST_FILENAME),
        type=str,
    )
    parser.add_argument(
        "--model_path",
