...
def train_o_net(
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[10, 25, 40], gamma=0.1
    )
    # load training image
    ...
    for cur_epoch in range(end_epoch):
        ...
        for batch_idx, (image, (gt_label, gt_bbox, gt_landmark)) in enumerate(
            train_data
        ):
            im_tensor = [
            ...
            landmark_loss = lossfn.landmark_loss(
                gt_label, gt_landmark, landmark_offset_pred
            )
            ...
            if batch_idx % frequent == 0:
                ...
                print(
                    "[%s, Epoch: %d, Step: %d] accuracy: %.6f, all_loss: %.6f, cls_loss: %.6f, bbox_reg_loss: %.6f, landmark_loss: %.6f, lr: %.6f"
                    % (
                        ...
                        cls_loss.data.tolist(),
                        box_offset_loss.data.tolist(),
                        landmark_loss.data.tolist(),
                        scheduler.get_lr()[0],
                    )
                )
        ...
        if (cur_epoch + 1) % 10 == 0:
            torch.save(
                net.state_dict(),
                os.path.join(
                    model_store_path, "onet_model_epoch_%d.pt" % (cur_epoch + 1)
                ),
            )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train ONet", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    parser.add_argument(
        "--anno_file",
        dest="annotation_file",
        help="training data annotation file",
        default=os.path.join(config.ANNO_STORE_DIR, config.ONET_TRAIN_IMGLIST_FILENAME),
        type=str,
    )
    parser.add_argument(
        "--model_path",
        dest="model_store_path",
        help="training model store directory",
        default=config.MODLE_STORE_DIR,
        type=str,
    )
    parser.add_argument(
        "--end_epoch",
