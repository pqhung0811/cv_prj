...
# -*- coding: utf-8 -*-
"""
Created on Sat May 26 14:38:08 2018
...
def gen_data(anno_file, data_dir, prefix):
    ...
    for annotation in annotations:
        ...
        for i in range(20):
            ...
            # cal iou
            iou = utils.IoU(
                crop_box.astype(np.float), np.expand_dims(gt_box.astype(np.float), 0)
            )
            if iou > 0.65:
                ...
                f.write(
                    save_file
                    + " -2 %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f \n"
                    % (
                        offset_x1,
                        offset_y1,
                        offset_x2,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="landmark dataset original annotation file",
        default=os.path.join(config.ANNO_STORE_DIR, "landmark_imagelist.txt"),
        type=str,
    )
    parser.add_argument(
        "--prefix_path",
        ...
        default="D:/CV/mtcnn_training/MTCNN_TRAIN/training_data/landmark_train",
        type=str,
    )
